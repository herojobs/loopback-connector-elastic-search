...
/*eslint no-console: ["error", { allow: ["trace","log"] }] */
    ...
    //Patch for infinate loop in log
    //https://github.com/elastic/elasticsearch-js/issues/278
...
ESConnector.prototype.buildFilter = function (modelName, idName, criteria, size, offset) {
  ...
  if (criteria) {
    // from HERO.Jobs - used to override the type in an incoming query
    if (criteria.indexType !== undefined && criteria.indexType !== null) {
      filter.type = criteria.indexType;
    }
...
ESConnector.prototype.count = function count(modelName, done, where) {
  ...
  if (where.indexType !== undefined && where.indexType !== null) {
    defaults.type = where.indexType;
  }
